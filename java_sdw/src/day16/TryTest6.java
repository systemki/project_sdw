...
public class TryTest6 {
	...
	/*   기 능	: 정수형 배열 index번지에 있는 값을 data로 설정하는 메소드
	 * 매개변수   : 정수형 배열 값  int arr[5] new arr {10,20,30,40,50} []arr, int index, int data
	 * 리턴타입   :  없음
	 * 메소드명   : setData
	 */
		public static void setData(int []arr, int index, int data) {
			...
			if(index < 0 || index >= arr.length) {
				throw new ArrayIndexOutOfBoundsException(index + "번지는 " + arr.length +"개 배열에서 유효하지 않은 배열의 번지 입니다");
			}
			arr[index] =data;
	...
	/*   기 능	: 정수형 배열에 index번지에 있는 값을 가져오는 메소드 
